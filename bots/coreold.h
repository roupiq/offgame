...
// {
//     string id = to_string(ndran() % 100);
//     // cerr << id << " id\n";
//     ofstream f("trash/matrix" + id + ".txt");
...
//             {
//                 b.push_back(read_double(out));
//                 // cerr << b.back() << " ";
//             }
//             // cerr << "\n";
//             payout = (read_double(out));
//             // cerr << payout << " payout\n";
//         }
//         else if (p == 1)
//         {
...
//             }
//             // cerr << "\n";
//             payout = (read_double(out)) / 100;
//             // cerr << payout << " payout\n";
//             break;
//         }
//         else
//         {
//             assert(!system(("cat trash/" + id + ".out").c_str()));
//             cerr << id << " id\n";
//             assert(!"!ups");
//         }
//     }
...
struct GameState
{
    ...
    long bfs(pii start) const
    {
        ...
        while (len(Q))
        {
            ...
            {
                pii u = pos + walks[i];
                if (boardf(u) == '#' || pos_has_bullet(u) || odw.count(u))
                    continue;
                Q.push(u);
                res += 1;
                odw.insert(u);
    ...
    long round_number() const
    {
        return round_num;
    }
    long achievable_squares() const
    {
        return bfs(r_pos);
    }
    long achievable_squares_enemy() const
    {
        return bfs(b_pos);
    }
    long number_of_bullets() const
    {
        return len(bullets) + start_bullets_n;
    }
    long distance_to_bullets(pii pos) const
    {
        long res = 0;
        for (int i = 0; i < 15; i++)
        {
            for (int j = 0; j < 20; j++)
            {
                res += long(pos_has_bullet({i, j}) * pow(manhat({i, j}, pos), 2));
            }
        }
        return res;
    }
    long square_distance_to_bullets() const
    {
        return distance_to_bullets(r_pos);
    }
    long enemy_square_distance_to_bullets() const
    {
        return distance_to_bullets(b_pos);
    }
    long distance_to_enemy() const
    {
        return manhat(b_pos, r_pos);
    }
    long distance_to_center() const
    {
        return manhat(r_pos, {10, 7});
    }
    long enemy_distance_to_center() const
    {
    ...
    unordered_set<pair<pii, pii>> cache;
    double rec_eval(int r, int d, int depth, bool burned) const
    {
        // if(!burned)
        // {
        //     cache.insert(posesi >= 4 && i != 8);
        // }
        if(depth == 0)
        {
    ...
    pair<Moves, Moves> eval_moves(int r, int d, int depth = 2) const
    {
        ...
        for (int i : ok)
        {
            for (int j : ok2)
            {
                GameState monte = check;
                monte.move_players(i, j);
                double score = monte.rec_eval(r, d, depth - 1, (i >= 4 && i != 8) || (j >= 4 && j != 8));
                eval[i][j] = score;
            }
        }
