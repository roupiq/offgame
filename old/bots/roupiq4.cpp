...
double eval(GameState &state)
{
    ...
    for (int k = 0; k < n_games; k++)
    {
        ...
        for (int j = 0; j < 25 && monte.round_num <= max_round_num; j++)
        {
    ...
    return (double)res / n_games;
}

pdi get_move(GameState state, int depth = -2)
{
    if (!depth)
        return {eval(state), 0};
    ...
    for (auto move_b : moves_b)
    {
        ...
        for (auto move_a : moves_a)
        {
            ...
            MAXX = max(MAXX, {get_move(next, depth + 1).x, move_a});
        }
        MINN = min(MAXX, MINN);
    }
    return MINN;
